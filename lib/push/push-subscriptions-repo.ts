...
export async function removeSubscriptionForClient(clientEmail: string, endpoint: string) { await getDb().delete(pushSubscriptions).where(and(eq(pushSubscriptions.clientEmail, clientEmail), eq(pushSubscriptions.endpoint, endpoint))); }
