...
export class D1FoodPlanDraftStore implements FoodPlanDraftStore {
  ...
  async save(record: SaveFoodPlanDraftRecord): Promise<void> {
    ...
    for (const meal of record.content.meals) {
      statements.push(this.database.prepare(
        `INSERT INTO nf_meals (public_id, plan_version_id, plan_day_id, title, scheduled_time, instructions, source_template_public_id, source_template_version_number, sort_order, created_at, updated_at) SELECT ?, version.id, (SELECT id FROM nf_plan_days WHERE public_id = ? AND plan_version_id = version.id), ?, ?, ?, NULL, NULL, ?, ?, ? FROM nf_plan_versions AS version WHERE version.id = (${scoped})`,
      ).bind(meal.publicId, meal.planDayPublicId, meal.title, meal.scheduledTime, meal.instructions, meal.sortOrder, record.updatedAt, record.updatedAt, ...scope));
