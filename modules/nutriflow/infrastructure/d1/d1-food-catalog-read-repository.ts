  ...
  async search(input: Parameters<FoodCatalogReadRepository["search"]>[0]): Promise<FoodCatalogSearchResultV1> {
    ...
    const result = await this.database.prepare(
      `SELECT food.public_id, revision.public_id AS revision_public_id, revision.revision_number,
              ...
              fiber_row.amount_scaled / 1000.0 AS fiber
       ...
       LEFT JOIN nf_food_nutrients AS fiber_row ON fiber_row.food_revision_id = revision.id AND fiber_row.nutrient_id = (SELECT id FROM nf_nutrients WHERE code = 'fiber')
