...
export default async function CheckInPage() {
  ...
  return (
    <PatientShell><main className="portal-shell checkin-page nf-experience-page nf-checkin-v2">
      ...
      <section className="nf-checkin-intro"><div><p className="section-kicker">Seu momento da semana</p><h1>{completedThisWeek ? "Check-in concluído." : "Como você está?"}</h1><p>{completedThisWeek ? "Suas respostas já estão disponíveis para o nutricionista." : "São cerca de 3 minutos, uma pergunta por vez."}</p></div><span className={completedThisWeek ? "is-done" : ""}>{completedThisWeek ? "Enviado ✓" : "Disponível"}</span></section>
