...
export default async function ProgressPage() {
  ...
  return <PatientShell><main className="portal-shell progress-page nf-experience-page nf-evolution-redesign">
    ...
    {!latest || !snapshot ? <><section className="evolution-empty"><p className="section-kicker">Evolução</p><h1>Seu acompanhamento começa aqui.</h1><p>Você ainda não tem avaliações registradas. Assim que seu nutricionista fizer sua primeira avaliação, ela aparecerá aqui.</p></section>{photoExperience}</> : <>
      ...
      {photoExperience}
      {assessments.length >= 2 ? <><EvolutionHistoryChart points={chartPoints} /><section className="evolution-report-action"><div><p className="section-kicker">Relatório profissional</p><h2>Leve sua evolução com você.</h2><p>Compare sua primeira avaliação com a mais recente em um documento organizado e pronto para impressão.</p></div><a className="button button-dark" href="/api/evolucao/relatorio" rel="noreferrer" target="_blank">Baixar relatório comparativo</a></section></> : circumference.length ? <section className="evolution-history evolution-single-measurements" aria-label="Circunferências atuais"><div className="evolution-section-heading"><div><p className="section-kicker">Circunferências</p><h2>Seu ponto de partida</h2><p className="evolution-history-note">Estas são as medidas da sua primeira avaliação. Na próxima, mostraremos as mudanças de forma simples e comparativa.</p></div></div><div className="evolution-measurement-grid">{circumference.map(([key, value]) => <article key={key}><small>{circumferenceLabels[key] ?? key}</small><strong>{number(Number(value))} <i>cm</i></strong></article>)}</div></section> : null}
    </>}
  </main></PatientShell>;
}
