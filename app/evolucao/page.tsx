...
export default async function ProgressPage() {
  ...
  return <PatientShell><main className="portal-shell progress-page nf-experience-page nf-evolution-redesign">
    ...
    {!latest || !snapshot ? <><section className="evolution-empty"><p className="section-kicker">Evolução</p><h1>Seu acompanhamento começa aqui.</h1><p>Você ainda não tem avaliações registradas. Assim que seu nutricionista fizer sua primeira avaliação, ela aparecerá aqui.</p></section>{photoExperience}</> : <>
      ...
      <section className="evolution-report-action"><div><p className="section-kicker">Relatório oficial</p><h2>{assessments.length === 1 ? "Guarde seu ponto de partida." : "Leve sua evolução com você."}</h2><p>O documento reúne os dados estruturados desta avaliação e o histórico disponível até ela.</p></div><a className="button button-dark" href={`/api/evolucao/relatorio?assessment=${encodeURIComponent(latest.publicId)}`} rel="noreferrer" target="_blank">Baixar relatório em PDF</a><div className="evolution-report-list" aria-label="Relatórios por avaliação">{assessments.toReversed().map((assessment) => <a href={`/api/evolucao/relatorio?assessment=${encodeURIComponent(assessment.publicId)}`} key={assessment.publicId} rel="noreferrer" target="_blank"><span>{date(assessment.capturedAt)}</span><strong>Baixar PDF</strong></a>)}</div></section>
    </>}
  </main></PatientShell>;
}
