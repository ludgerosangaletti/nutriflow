...
import { GeneratedPdfLauncher } from "../visualizar-documento/generated-pdf-viewer";
...
export default async function ProgressPage() {
  ...
  return <PatientShell><main className="portal-shell progress-page nf-experience-page nf-evolution-redesign">
    ...
    {!latest || !snapshot ? <><section className="evolution-empty"><p className="section-kicker">Evolução</p><h1>Seu acompanhamento começa aqui.</h1><p>Você ainda não tem avaliações registradas. Assim que seu nutricionista fizer sua primeira avaliação, ela aparecerá aqui.</p></section>{photoExperience}</> : <>
      ...
      {circumference.length && previousSnapshot ? <details className="evolution-disclosure"><summary>Circunferências <span>⌄</span></summary><div className="evolution-detail-grid">{circumference.map(([key, value]) => { const prior = previousSnapshot.input.circumferencesCm?.[key]; return <article key={key}><small>{circumferenceLabels[key] ?? key}</small><strong>{number(Number(value))} cm</strong>{typeof prior === "number" ? <span>{delta(Number(value) - prior, "cm")}</span> : null}</article>; })}</div></details> : null}
      {photoExperience}
      {assessments.length >= 2 ? <EvolutionHistoryChart points={chartPoints} /> : circumference.length ? <section className="evolution-history evolution-single-measurements" aria-label="Circunferências atuais"><div className="evolution-section-heading"><div><p className="section-kicker">Circunferências</p><h2>Seu ponto de partida</h2><p className="evolution-history-note">Estas são as medidas da sua primeira avaliação. Na próxima, mostraremos as mudanças de forma simples e comparativa.</p></div></div><div className="evolution-measurement-grid">{circumference.map(([key, value]) => <article key={key}><small>{circumferenceLabels[key] ?? key}</small><strong>{number(Number(value))} <i>cm</i></strong></article>)}</div></section> : null}
      <section className="evolution-report-action"><div><p className="section-kicker">Relatório oficial</p><h2>{assessments.length === 1 ? "Guarde seu ponto de partida." : "Leve sua evolução com você."}</h2><p>O documento reúne os dados estruturados desta avaliação e o histórico disponível até ela.</p></div><GeneratedPdfLauncher backHref="/evolucao" className="button button-dark" filename={`avaliacao-fisica-${latest.capturedAt.slice(0, 10)}.pdf`} pdfUrl={`/api/evolucao/relatorio?assessment=${encodeURIComponent(latest.publicId)}`} title="Avaliação física">Abrir relatório em PDF</GeneratedPdfLauncher><div className="evolution-report-list" aria-label="Relatórios por avaliação">{assessments.toReversed().map((assessment) => <GeneratedPdfLauncher backHref="/evolucao" filename={`avaliacao-fisica-${assessment.capturedAt.slice(0, 10)}.pdf`} key={assessment.publicId} pdfUrl={`/api/evolucao/relatorio?assessment=${encodeURIComponent(assessment.publicId)}`} title="Avaliação física"><span>{date(assessment.capturedAt)}</span><strong>Abrir PDF</strong></GeneratedPdfLauncher>)}</div></section>
    </>}
  </main></PatientShell>;
}
