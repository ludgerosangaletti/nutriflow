...
const skinField = (key: (typeof skin)[number]) => `skinfold_${key}`;
const circumferenceField = (key: (typeof circ)[number]) => `circumference_${key}`;
...
export default function ClinicalAssessmentForm({ email, assessment }: { email: string; assessment?: EditableAssessment }) {
  ...
  const inputFrom = (form: HTMLFormElement) => {
    ...
    const optional = Object.fromEntries(circ.flatMap((key) => {
      const raw = String(values.get(circumferenceField(key)) ?? "").trim();
      return raw ? [[key, Number(raw)]] : [];
    }));
    return { sex: String(values.get("sex")) as "male" | "female", age: number("age"), weightKg: number("weightKg"), heightCm: number("heightCm"), measurementSide: String(values.get("measurementSide")) as "left" | "right", skinfoldsMm: Object.fromEntries(skin.map((key) => [key, number(skinField(key))])), circumferencesCm: optional };
  };
