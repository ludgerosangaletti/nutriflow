...
export default function ClinicalAssessmentForm({ email }: { email: string }) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/admin/clinical-assessments", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, mode: "save", capturedAt: calculation.capturedAt, input }) });
    const data = await response.json().catch(() => ({})); setSaving(false);
    setMessage(response.ok ? data.duplicate ? "Esta avaliação já estava registrada." : "Avaliação registrada com segurança." : data.error || "Não foi possível registrar.");
