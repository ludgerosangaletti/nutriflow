  ...
  return <div className="nutriflow-editor">
    ...
    <div className={`nutriflow-editor-layout ${tools.catalogEnabled || tools.recipesEnabled || tools.mealTemplatesEnabled ? "has-library" : ""}`}>
      ...
      <div className="nutriflow-tools-rail">
        ...
        <aside className="nutriflow-sidebar"><div><span>Paciente</span><strong>{patientName}</strong><small>Rascunho · versão {draft.versionNumber}.{draft.revision}</small></div><section className="nutriflow-admin-macros" aria-label="Macros totais da estratégia"><header><span>Totais da estratégia</span><strong>{activeDay?.label ?? "Sem estratégia"}</strong></header><div className="nutriflow-admin-macro-grid"><article><small>Valor calórico</small><strong>{macroSummary.totals.energyKcal == null ? "—" : `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 0 }).format(macroSummary.totals.energyKcal)} kcal`}</strong></article><article><small>Proteínas</small><strong>{macroSummary.totals.protein == null ? "—" : `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 1 }).format(macroSummary.totals.protein)} g`}</strong></article><article><small>Carboidratos</small><strong>{macroSummary.totals.carbohydrate == null ? "—" : `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 1 }).format(macroSummary.totals.carbohydrate)} g`}</strong></article><article><small>Gorduras</small><strong>{macroSummary.totals.fat == null ? "—" : `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 1 }).format(macroSummary.totals.fat)} g`}</strong></article></div><p>{macroSummary.itemCount === 0 ? "Adicione alimentos da biblioteca para iniciar o cálculo." : macroSummary.complete ? `Cálculo completo da configuração selecionada em ${macroSummary.itemCount} alimento(s).` : `Cálculo parcial: ${macroSummary.completeItemCount} de ${macroSummary.itemCount} alimento(s) possuem todos os macros.`}</p></section><label><span>Observações gerais</span><textarea maxLength={4000} placeholder="Contexto clínico, estratégia e pontos importantes para revisão." value={draft.planNotes ?? ""} onChange={(event) => mutate((current) => ({ ...current, planNotes: event.target.value || null }))} /></label><section><span>Estrutura atual</span><dl><div><dt>Estratégias</dt><dd>{draft.content.days.length}</dd></div><div><dt>Refeições</dt><dd>{draft.content.meals.length}</dd></div><div><dt>Alimentos</dt><dd>{totalOptionItems}</dd></div></dl></section><button className="nutriflow-save-now" type="button" disabled={syncState === "saving" || syncState === "saved"} onClick={saveNow}>{syncState === "saving" ? "Salvando…" : "Salvar agora"}<small>Ctrl/⌘ + S</small></button><button className="nutriflow-publish" type="button" disabled={syncState !== "saved" || !allMealOptionsReady} onClick={publishDraft}>Publicar para o paciente<small>Versão imutável e auditada</small></button><p>{allMealOptionsReady ? "A publicação só ficará visível quando a flag do paciente for homologada." : "Preencha ao menos um alimento em cada opção cadastrada antes de publicar."}</p></aside>
      </div>
    </div>
  </div>;
}
