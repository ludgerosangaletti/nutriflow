...
import { buildTrainingAnamnesisSummary } from "./training-anamnesis-summary.ts";
...
function Choices({ options, value, onChange, multiple = false, compact = false }: Readonly<{ options: readonly (readonly [string, string])[]; value: string | readonly string[] | null; onChange: (value: string | readonly string[]) => void; multiple?: boolean; compact?: boolean }>) {
  const selected = Array.isArray(value) ? value : value ? [value] : [];
  return <div className={`training-anamnesis-choices${compact ? " is-compact" : ""}`}>{options.map(([key, label]) => <button key={key} type="button" aria-pressed={selected.includes(key)} className={selected.includes(key) ? "is-selected" : ""} onClick={() => {
...
export default function TrainingAnamnesisForm({ initial }: Readonly<{ initial: TrainingAnamnesisV1 }>) {
  ...
  const [messageTone, setMessageTone] = useState<"error" | "saved">("error");
  const [submitted, setSubmitted] = useState<TrainingAnamnesisV1 | null>(null);
  ...
  useEffect(() => {
    if (messageTone !== "saved" || !message) return;
    const timer = window.setTimeout(() => setMessage(""), 2200);
    return () => window.clearTimeout(timer);
  }, [message, messageTone]);

  async function save(submit: boolean) {
    if (!valid) { setMessageTone("error"); setMessage("Responda os campos desta etapa para continuar."); return false; }
    ...
    try {
      ...
      if (submit) setSubmitted(result.data);
      else { setMessageTone("saved"); setMessage("Salvo — você pode continuar depois."); }
      return true;
    } catch (error) { setMessageTone("error"); setMessage(error instanceof Error ? error.message : "Não foi possível salvar suas respostas."); return false; }
  ...
  if (submitted) {
    const summary = buildTrainingAnamnesisSummary(submitted.answers);
    const receivedAt = submitted.submittedAt
      ? new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(submitted.submittedAt))
      : "agora";
    return <main className="training-anamnesis-screen"><section className="training-anamnesis-complete">
      <span aria-hidden="true">✓</span><p>NutriFlow Training</p><h1>Anamnese recebida.</h1>
      <p>Organizamos os pontos principais que você enviou.</p>
      {summary.length ? <div className="training-anamnesis-summary"><h2>Suas respostas</h2>{summary.map((item) => <article key={item.label} className={item.attention ? "is-attention" : ""}><strong>{item.label}</strong><p>{item.value}</p>{item.detail ? <small>{item.detail}</small> : null}</article>)}</div> : null}
      <ol className="training-anamnesis-timeline"><li className="is-done"><i>✓</i><span><strong>Anamnese recebida</strong><small>hoje, {receivedAt}</small></span></li><li><i>2</i><span><strong>Montagem da rotina</strong><small>feita a partir das suas respostas</small></span></li><li><i>3</i><span><strong>Treino disponível</strong><small>você recebe um aviso quando estiver pronto</small></span></li></ol>
      <Link href="/treino">Continuar para o Training</Link>
      <button type="button" onClick={() => setSubmitted(null)}>Revisar minhas respostas</button>
    </section></main>;
  }

  return <main className="training-anamnesis-screen">
    <header className="training-anamnesis-header"><Link href="/treino">← Voltar</Link><div><p>NutriFlow Training</p><h1>{steps[step][0]}</h1><span>{steps[step][1]}</span></div><small>Etapa {step + 1} de {steps.length}</small><div className="training-anamnesis-progress" role="progressbar" aria-valuemin={1} aria-valuemax={steps.length} aria-valuenow={step + 1}><i style={{ width: `${progress}%` }} /></div></header>
    <section className="training-anamnesis-form">
      {step === 0 ? <>
        ...
        <Field title="Existe alguma região que gostaria de desenvolver com maior prioridade?" hint="Você pode selecionar mais de uma."><Choices compact multiple options={TRAINING_ANAMNESIS_PRIORITIES} value={answers.priorities} onChange={(value) => { const next = value as TrainingAnamnesisAnswersV1["priorities"]; update({ priorities: next.includes("none") && !answers.priorities.includes("none") ? ["none"] : next.filter((item) => item !== "none") }); }} />{answers.priorities.includes("other") ? <input className="training-anamnesis-reveal" value={answers.priorityOther ?? ""} onChange={(event) => update({ priorityOther: event.target.value })} placeholder="Qual outra região?" /> : null}</Field>
      </> : null}
      {step === 1 ? <>
        <Field title="Há quanto tempo pratica musculação?"><Choices compact options={TRAINING_ANAMNESIS_EXPERIENCE} value={answers.experience} onChange={(value) => update({ experience: value as TrainingAnamnesisAnswersV1["experience"] })} /></Field>
      ...
      {step === 2 ? <>
        ...
        <Field title="Quanto tempo normalmente possui para cada treino?"><Choices compact options={TRAINING_ANAMNESIS_DURATIONS} value={answers.sessionDuration} onChange={(value) => update({ sessionDuration: value as TrainingAnamnesisAnswersV1["sessionDuration"] })} /></Field>
      </> : null}
      {step === 3 ? <>
      ...
      {message ? <p className={`training-anamnesis-message${messageTone === "saved" ? " is-saved" : ""}`} role={messageTone === "error" ? "alert" : "status"}>{message}</p> : null}
