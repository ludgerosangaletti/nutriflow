...
export default async function TrainingPage() {
  ...
  if (!portal.publication && portal.anamnesis.status !== "submitted") return <PatientShell><main className="training-patient-screen"><section className="training-patient-rest training-anamnesis-call"><p>NutriFlow Training</p><h1>Complete sua anamnese de treino</h1><p>Precisamos conhecer um pouco melhor sua rotina, experiência e preferências antes de preparar sua prescrição.</p><Link href="/treino/anamnese">Responder anamnese</Link><Link className="is-secondary" href="/area-cliente">Voltar ao início</Link></section></main></PatientShell>;
  if (!portal.publication) return <PatientShell><main className="training-patient-screen"><section className="training-patient-rest"><p>NutriFlow Training</p><h1>Seu treino está sendo preparado</h1><p>Assim que a rotina for publicada, ela aparecerá aqui.</p><Link href="/treino/anamnese">Atualizar informações do treino</Link><Link className="is-secondary" href="/area-cliente">Voltar ao início</Link></section></main></PatientShell>;
