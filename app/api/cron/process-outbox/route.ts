...
const prioritizedEventTypes = Object.freeze([...new Set(handlers.map(({ eventType }) => eventType))]);
...
export async function POST(request: Request) { if (!safeEqual(request.headers.get("x-checkin-reminder-secret") || "", env.CHECKIN_REMINDER_SECRET || "")) return Response.json({ error: "unauthorized" }, { status: 401 }); const dispatcher = new ReliableDomainEventDispatcher(new D1EventConsumptionRepository(env.DB), handlers, () => crypto.randomUUID()); const processor = new OutboxProcessor(new D1OutboxRepository(env.DB, prioritizedEventTypes), dispatcher, () => crypto.randomUUID()); const results = []; // pg_net has a 5-second request budget; process one event per tick for a reliable 200 response and let the next cron tick continue the backlog. Notification-bearing events are claimed first so draft autosaves cannot delay patient communications.
