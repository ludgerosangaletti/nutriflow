...
import { createNutriFlowApiErrorV1, NutriFlowContractError, parseArchiveReusableContentCommandV1, parseSaveRecipeCommandV1, parseSearchReusableContentQueryV1 } from "../../../../../modules/nutriflow/contracts/v1/validation.ts";
...
function failure(error: unknown, correlation: string, startedAt: number) {
  ...
  console.error("[nutriflow.recipes.api]", JSON.stringify({ correlationId: correlation, errorCode: NUTRIFLOW_ERROR_CODES.INTERNAL_ERROR }));
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await auth.operations.searchRecipes({ actor: auth.context.actor, organizationId: auth.context.organizationId, organizationPublicId: auth.context.organizationPublicId, clientId, query });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const command = parseSaveRecipeCommandV1({ ...record(body.command), apiVersion: NUTRIFLOW_API_VERSION, correlationId: correlation });
    const key = request.headers.get("idempotency-key")?.trim(); if (!key) throw new NutriFlowContractError("idempotencyKey");
    const result = await auth.operations.saveRecipe({ actor: auth.context.actor, organizationId: auth.context.organizationId, organizationPublicId: auth.context.organizationPublicId, clientId, command, idempotencyKey: key, requestHash: await sha256Json({ clientId, command }) });
    return Response.json({ apiVersion: NUTRIFLOW_API_VERSION, correlationId: result.correlationId, data: result.data }, { status: 201, headers: headers(startedAt, command.recipePublicId ? "2+1-batch" : "1-batch") });
  } catch (error) { return failure(error, correlation, startedAt); }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const key = request.headers.get("idempotency-key")?.trim(); if (!key) throw new NutriFlowContractError("idempotencyKey");
    const result = await auth.operations.archiveRecipe({ actor: auth.context.actor, organizationId: auth.context.organizationId, organizationPublicId: auth.context.organizationPublicId, clientId, command, idempotencyKey: key, requestHash: await sha256Json({ clientId, command }) });
