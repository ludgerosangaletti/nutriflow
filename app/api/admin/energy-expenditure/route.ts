...
export async function POST(request: Request) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO nf_outbox_events (event_id, organization_id, event_type, event_version, aggregate_type, aggregate_public_id, aggregate_version, actor_auth_user_id, correlation_id, causation_id, occurred_at, payload_json, metadata_json, status, attempts, available_at) VALUES (?, ?, ?, 1, ?, ?, 1, ?, ?, NULL, ?, ?, ?, 'pending', 0, ?)").bind(publicId("event"), context.organizationId, "energy-expenditure.calculated", "energy-expenditure-calculation", calculationId, admin.id, correlationId, occurredAt, JSON.stringify({ publicId: calculationId, clientId: client.id, totalKcal: result.totalKcal }), JSON.stringify({ source: "admin-clinical-calculator" }), occurredAt),
  ]);
