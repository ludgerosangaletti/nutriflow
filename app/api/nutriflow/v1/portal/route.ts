...
export async function POST(request: Request) {
  ...
  try {
    ...
    const command = parseRecordPatientPortalViewCommandV1(await request.json());
    const result = await recordNutriFlowPatientPortalView({
      context,
      publicationPublicId: command.publicationPublicId,
      correlationId: correlation,
    });
    return Response.json(
      { apiVersion: NUTRIFLOW_API_VERSION, correlationId: correlation, recorded: result.recorded },
      { headers: headers(startedAt, 4) },
    );
  } catch (error) {
    if (error instanceof NutriFlowContractError || error instanceof SyntaxError) {
      return Response.json(createNutriFlowApiErrorV1(NUTRIFLOW_ERROR_CODES.INVALID_INPUT, correlation), { status: 400, headers: headers(startedAt, 3) });
    }
    if (error instanceof NutriFlowApplicationError) {
      return Response.json(createNutriFlowApiErrorV1(error.code, correlation), { status: error.httpStatus, headers: headers(startedAt, 4) });
    }
    console.error("[nutriflow.portal-view]", JSON.stringify({ correlationId: correlation, errorCode: NUTRIFLOW_ERROR_CODES.INTERNAL_ERROR }));
    return Response.json(createNutriFlowApiErrorV1(NUTRIFLOW_ERROR_CODES.INTERNAL_ERROR, correlation), { status: 500, headers: headers(startedAt, 4) });
  }
}
