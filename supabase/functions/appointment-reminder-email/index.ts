...
Deno.serve(async (request) => {
  ...
  const humanPhone = "5542999889176";
  ...
  const confirmText = encodeURIComponent(
    `Olá, Ludgero! Sou ${body.name}. Confirmo meu retorno agendado para ${appointment}.`,
  );
  const rescheduleText = encodeURIComponent(
    `Olá, Ludgero! Sou ${body.name}. Preciso alinhar outro horário para meu retorno previsto em ${appointment}.`,
  );
...
<p style="font-size:16px;line-height:1.6;margin-bottom:24px">Escolha uma opção para falar diretamente comigo pelo WhatsApp:</p>
<a href="https://wa.me/${humanPhone}?text=${confirmText}" style="display:inline-block;background:#ffeb00;color:#111;text-decoration:none;font-weight:800;padding:15px 20px;border-radius:10px;margin:0 8px 10px 0">Confirmar retorno</a>
<a href="https://wa.me/${humanPhone}?text=${rescheduleText}" style="display:inline-block;background:#f1f0ea;color:#111;text-decoration:none;font-weight:800;padding:15px 20px;border-radius:10px;margin-bottom:10px">Solicitar outro horário</a>
<p style="font-size:13px;line-height:1.5;color:#777;margin:24px 0 0">O horário será considerado confirmado após o alinhamento pelo WhatsApp.</p>
...
<tr><td style="padding:34px"><p style="font-size:16px;line-height:1.6;margin-top:0">O paciente recebeu o lembrete de confirmação e foi orientado a falar com você pelo WhatsApp humano.</p>
